...
const Corrente1 = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const dadosFiltrados = data.filter(motor => motor.motorid === 'Motor01');
      if (dadosFiltrados.length > 0) {
        setSensorData(dadosFiltrados);
        setIsLoading(false);
      } else {
        console.error('Nenhum dado encontrado para MotorID: Motor01.');
        setIsLoading(false);
      }
    } catch (error) {
        ...
        const newChartInstance = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              {
                label: 'Corrente',
                data: sensorData.map(entry => entry.corrente),
